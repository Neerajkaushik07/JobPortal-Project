...
// Backend endpoint to fetch top profiles based on experience
exports.topprofiles= async (req, res) => {
  try {
      const minExperience = req.query.minExperience; // Minimum experience level required
      // Fetch top profiles based on experience level, limit to 10
      const topProfiles = await User.find({ experience: { $gte: minExperience } }).limit(10);

      res.status(200).json({ success: true, topProfiles });
  } catch (error) {
      console.error(error);
      res.status(500).json({ success: false, message: "Failed to fetch top profiles" });
  }
}